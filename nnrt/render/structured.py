...
def format_structured_output(
    ...
    if statements_by_type.get('quote') or (hasattr(metadata, 'speech_acts') if metadata else False):
        from nnrt.validation import InvariantSeverity, check_quote_has_speaker
        ...
        if validated_quotes:
            ...
            for quote in validated_quotes:
                ...
                if is_nested:
                    ...
                    lines.append("      (nested quote - attribution may need review)")
                else:
    ...
    if events:
        ...
        len(quarantined_events)
        if total > 0:
            lines.append(f"  📊 Event Validation: {passed}/{total} passed ({100*passed//total}%)")
            lines.append("  ⚠️ Validated events list disabled pending event extraction fixes")
            lines.append("")
